...
const calculateTotalHarga = (trans) => {
    const totalSubTotal = trans.products.reduce((acc, product) => acc + product.pivot.Sub_Total, 0);
...
const PdfGenerator = (trans) => {
    ...
    //Atur Data Customer
    pdf.setFont('custom', 'bold');
    pdf.text(`Customer  : ${trans.tblcustomer.email} / ${trans.tblcustomer.Nama_Customer}`, 13, 72);
    pdf.setFont('custom', 'normal');
    ...
    pdf.text(`Total                                                                                            ${subTotal}`, 13, pdf.previousAutoTable.finalY + 10);
    pdf.text(`Ongkos Kirim (rad. ${trans.tblalamat.Jarak} km)                                                           ${trans.tblalamat.Biaya}`, 13, pdf.previousAutoTable.finalY + 17);
    pdf.text(`Total                                                                                            ${subTotal - trans.tblalamat.Biaya}`, 13, pdf.previousAutoTable.finalY + 24);
    const usedPoints = calculateUsedPoints(trans);
    pdf.text(`Potongan ${usedPoints} poin                                                                     ${usedPoints * 100}`, 13, pdf.previousAutoTable.finalY + 32);
